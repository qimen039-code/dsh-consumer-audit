...
export function audit(input, options = {}) {
  ...
  for (const row of rows) {
    ...
    for (const tool of reg.tools ?? []) {
      ...
      if (n === 0) {
        ...
        consumed.push({ kind: "tool", object: tool, from: row.name, consumer_count: n });
      }
    }
  }
  ...
  for (const skill of skillsOnDisk) {
    ...
    if (n === 0) {
      ...
      consumed.push({ kind: "skill", object: skill.name, from: skill.preset, consumer_count: n, offers: offered });
    }
  }
  ...
  return {
    ...
    boundaries: [
      ...
      "A skill load is counted from a skill tool call naming it. That is evidence the skill was delivered, not that its content changed what the session did; no deterministic log signal establishes the latter, so this report never claims it.",
