...
export function audit(input, options = {}) {
  ...
  const invocationErrors = countConsumers ? (input.invocationErrors ?? {}) : {};
  const skillLoadErrors = countConsumers ? (input.skillLoadErrors ?? {}) : {};
  const skillOffers = countConsumers ? (input.skillOffers ?? {}) : {};

  const findings = [];
  const unassessed = [];
  const notes = [];
  // Every registered capability that DID see use, with the count. Reporting only
  // the zeros answers "is this dead" but not "how much is it used".
  const consumed = [];
  ...
  for (const row of rows) {
    ...
    for (const tool of reg.tools ?? []) {
      ...
      const failed = invocationErrors[tool] ?? 0;
      const succeeded = n - failed;
      if (n > 0 && succeeded === 0) {
        findings.push({
          kind: "tool_never_delivered",
          object: tool + " (from " + row.name + ")",
          detail: "the tool was called " + n + " time(s) and every result came back with isError, so no call delivered anything",
          consumer_count: n,
          failed_count: failed,
          evidence: {
            locator: pkg.dir,
            method: "tool name \"" + tool + "\" paired with its result records across " + (input.sessionsScanned ?? 0) + " scanned session logs, using the isError flag",
          },
          falsifier: "find one non-error result for \"" + tool + "\" in a session log this run did not scan",
        });
      } else if (n === 0) {
        findings.push({
        ...
        consumed.push({ kind: "tool", object: tool, from: row.name, consumer_count: n, attempts: n, failed, succeeded });
      }
    }
  }
  ...
  for (const skill of skillsOnDisk) {
    ...
    const failed = skillLoadErrors[skill.name] ?? 0;
    const succeeded = n - failed;
    const offered = skillOffers[skill.name] ?? 0;
    if (n > 0 && succeeded === 0) {
      findings.push({
        kind: "skill_never_delivered",
        object: skill.name + " (" + skill.preset + ")",
        detail: "the skill tool was called with this name " + n + " time(s) and every call came back with isError, so the skill body was never delivered",
        consumer_count: n,
        failed_count: failed,
        offers: offered,
        evidence: {
          locator: skill.path,
          method: "skill tool calls carrying the name \"" + skill.name + "\" paired with their results and the isError flag",
        },
        falsifier: "find one non-error result for a skill call naming \"" + skill.name + "\"",
      });
    } else if (n === 0) {
      findings.push({
        ...
        detail:
          offered > 0
            ? `the skill file exists and was listed to the model in ${offered} catalog injection(s), but the skill tool was never called with its name`
            : "the skill file exists on disk and was neither listed nor loaded in the scanned sessions",
        consumer_count: 0,
        offers: offered,
        evidence: {
          locator: skill.path,
          method: `skill tool calls carrying the name "${skill.name}" searched across ${input.sessionsScanned ?? 0} scanned session logs`,
        },
        falsifier: `find one skill tool call naming "${skill.name}" in a session log this run did not scan`,
      });
    } else {
      consumed.push({ kind: "skill", object: skill.name, from: skill.preset, consumer_count: n, attempts: n, failed, succeeded, offers: offered });
    }
  }

  // A skill can be offered by the host without its file living in any root this
  // run searched: project-level and user-level skill roots are not scanned.
  // Saying so is better than leaving the name out of the report entirely.
  const onDisk = new Set(skillsOnDisk.map((s) => s.name));
  for (const [name, offers] of Object.entries(skillOffers)) {
    if (onDisk.has(name) || name === "skill") continue;
    const loads = skillLoads[name] ?? 0;
    notes.push({
      kind: "skill_outside_searched_roots",
      object: name,
      detail: `listed to the model in ${offers} catalog injection(s) and loaded ${loads} time(s), but no SKILL.md for it exists in the searched roots`,
      evidence: {
        locator: (input.presetRoots ?? []).map((r) => r.dir ?? `${r.origin}: not resolved`).join(" | "),
        method: "skill name matched against catalog listings and skill tool calls",
      },
    });
  }
  ...
  return {
    ...
    generated_from: {
      ...
      // Which roots were actually searched for skills. A root that was never
      // resolved means its skills are absent from this report, not that they
      // are unused.
      // Which packages declared their names and which were guessed. A declared
      // list is authoritative for its package; a scanned one can misread a name.
      capability_names: Object.fromEntries(
        Object.entries(packages)
          .filter(([, p]) => p && p.resolved !== false)
          .map(([name, p]) => [name, p.method ?? "static"]),
      ),
      preset_roots_searched: (input.presetRoots ?? []).map((r) => ({
        origin: r.origin,
        dir: r.dir,
        searched: Boolean(r.exists),
        ...(r.note ? { note: r.note } : {}),
      })),
    },
    findings,
    // Registered capabilities that did see use, with their counts. A finding
    // answers "is this dead"; this answers "how much is it used".
    consumed: consumed.sort((a, b) => b.consumer_count - a.consumer_count),
    notes,
    unassessed,
    ...
    boundaries: [
      ...
      "Three levels are kept apart. Registered means declared. Called means a call record exists. Delivered means the paired result did not carry isError. This report stops at delivered.",
      "Whether a delivered capability did what it was designed to do is a judgement, and it belongs to the model reading this report, not to this tool. Use the evidence action to pull the actual arguments and result text for a capability, then read them against what that capability is supposed to do.",
      "Counts cover the logs that were scanned. A capability used elsewhere is not covered.",
      "A skill is offered when a catalog injection lists it. Offered and loaded are counted separately because being listed is much weaker than being used.",
