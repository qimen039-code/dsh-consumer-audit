...
/** tool name -> observed call count, plus skill name -> observed load count. */
export function countConsumers(dshHome) {
  ...
  walkDirs(sessionsRoot, 3, (file) => {
    ...
    for (const r of records) {
      ...
      // A skill is consumed when it is listed to the model (catalog) or loaded by name.
      if (t === "user/message" && r?.data?.source?.kind === "skill-catalog") {
        const text = JSON.stringify(r.data);
        for (const m of text.matchAll(/`([a-z0-9][a-z0-9-]{2,})`/g)) {
          skillLoads[m[1]] = (skillLoads[m[1]] ?? 0) + 1;
        }
      }
    }
  });
  return { invocations, skillLoads, sessionsScanned, unreadable, recordsSeen };
}
...
/** Static scan of one installed package. Declared as a heuristic, not a proof. */
export function scanPackage(dir, maxFiles = 400) {
  ...
  return { registrations, scanned, dir };
}
...
export function findSkillsOnDisk(dshHome, shippedPresetsDir) {
  ...
  if (existsSync(userPresets)) roots.push({ dir: userPresets, origin: "user preset" });
  if (shippedPresetsDir && existsSync(shippedPresetsDir)) {
    roots.push({ dir: shippedPresetsDir, origin: "shipped preset" });
  }
...
export function collect({ dshHome, profileDir, shippedPresetsDir, countConsumersToo = true }) {
  ...
  const skillsOnDisk = findSkillsOnDisk(dshHome, shippedPresetsDir);
  return {
    rows,
    packages,
    searchedRoots,
