...
插件的工具和 skill 是给模型调用的。注册成功不代表模型真的调用过它们，而这件事没有任何地方会报告。这个插件负责把它报出来。
...
模型在几种情形下会发起这次检查：刚装完一个插件，需要确认它的能力是否真的会被用到；怀疑某段功能写了但没接上；清理 profile 之前要判断删掉哪些不会有影响；自己写插件时想知道哪部分没有被调用过。

这几次检查都由模型判断并发起，不需要手敲命令。
...
报告由调用它的模型读取，同时也会留在会话转录里。报告里有六类字段。
